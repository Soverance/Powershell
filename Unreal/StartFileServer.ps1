...
# This script starts the Unreal cook server for Xbox One, and allows it to accept connections for debugging Xbox games via Visual Studio
...
# Set command line arguments under "Debugging" to [MapName] -filehostip=127.0.0.1

# FOR PLAYSTATION 4:
# Set Executable Arguments under "Debugging" to the following:
# [RelativePathToProject.uproject] [MapName] -filehostip=192.168.0.1
# e.g. ../../../MyProject/MyProject.uproject -filehostip=192.168.0.1 (or whatever the IP address of your PC is as seen by the PS4)
...
{
    # Run the cook server and prepare it to accept connections on the local host
	./UE4Editor-cmd.exe Ethereal -run=cook -targetplatform=PS4 -cookonthefly
}

function StartServer ()
{
    # Navigate to the local path where the Unreal tools are located
	cd U:/UnrealEngine/Engine/Binaries/Win64
